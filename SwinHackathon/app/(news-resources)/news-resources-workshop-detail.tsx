...
export default function NewsWorkshopDetailScreen() {
  const { colors } = useTheme();
  const { scale, verticalScale, scaleFont } = useResponsive();
  const router = useRouter();
  const params = useLocalSearchParams<{ workshopId?: string }>();
  const workshop = workshopFeed.find((item) => item.id === params.workshopId) ?? workshopFeed[0];
  const moreWorkshops = workshopFeed.filter((item) => item.id !== workshop.id).slice(0, 2);
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.backgroundSoft }]} edges={['top']}>
      ...
      >
        <CommunityScreenHeader title="Workshop Detail" onBack={() => router.back()} />
        ...
        <CommunityCard style={[styles.heroCard, { backgroundColor: colors.primaryLight }]}>
          ...
          >
            {workshop.instructor} • {workshop.schedule}
          </Text>
          ...
          <Pressable
            ...
            onPress={() => undefined}
          >
            <Text style={[styles.primaryButtonText, { color: colors.card }]}>Reserve Workshop Seat</Text>
          </Pressable>
        </CommunityCard>
        ...
        <CommunityCard>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>More sessions</Text>
            <Pressable onPress={() => router.replace('/news-resources-workshops')}>
              <Text style={[styles.sectionLink, { color: colors.primaryDark }]}>All workshops</Text>
            </Pressable>
          </View>

          <View style={styles.relatedStack}>
            {moreWorkshops.map((item) => (
              <Pressable
                key={item.id}
                style={[styles.relatedRow, { borderBottomColor: colors.border }]}
                onPress={() =>
                  router.replace({
                    pathname: '/news-resources-workshop-detail',
                    params: { workshopId: item.id },
                  })
                }
              >
                <View style={styles.relatedText}>
                  <Text style={[styles.relatedMeta, { color: colors.primaryDark }]}>{item.schedule}</Text>
