...
export default function SmartBudgetDetailsScreen() {
    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: colors.backgroundSoft }]} edges={['top', 'bottom']}>
      <View style={styles.content}>
        <View style={styles.headerRow}>
          <Pressable style={[styles.headerButton, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={() => goSmartBudgetBack(router, '/(finance)/smart-budgeting/setup/stepper')}>
            <MaterialIcons name="arrow-back" size={20} color={colors.text} />
          </Pressable>
          <View style={[styles.progressBar, { backgroundColor: colors.border }]}>
            <View style={[styles.progressValue, { backgroundColor: colors.primaryDark, width: '55%' }]} />
          </View>
          <View style={styles.headerSpacer} />
        </View>

        <Text style={[styles.eyebrow, { color: colors.primaryDark }]}>Budget Details</Text>
        <Text style={[styles.title, { color: colors.text }]}>Fill in the essential budget details.</Text>
        <Text style={[styles.body, { color: hexToRgba(colors.text, 0.58) }]}>
          A compact version of the detailed form screens shown before the dashboard is generated.
        </Text>

        <View style={styles.fieldStack}>
          {[
            ['Primary income', '$4,800'],
            ...
            <View key={label} style={styles.fieldGroup}>
              ...
              <View style={[styles.inputWrap, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <TextInput defaultValue={value} style={[styles.input, { color: colors.text }]} />
              </View>
            </View>
          ))}
        </View>
        ...
        <View style={styles.bottomArea}>
          <Pressable style={[styles.primaryButton, { backgroundColor: colors.primaryDark }]} onPress={() => router.push('/(finance)/smart-budgeting/setup/housing')}>
...
function createStyles(colors: ColorTheme) {
  return StyleSheet.create({
    ...
    fieldStack: { marginTop: 24, gap: 16 },
    fieldGroup: { gap: 8 },
    ...
    input: { fontSize: Typography.body, fontWeight: '700' },
