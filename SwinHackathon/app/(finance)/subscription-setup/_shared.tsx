...
export function SubscriptionSetupShell({
  step,
  totalSteps,
  title,
  body,
          ...
          <Pressable
            ...
            onPress={
              ...
              (() => {
                ...
                router.replace('/(finance)/subscriptions');
              })
            }
          >
...
export function SubscriptionSetupPrimaryButton({ label, onPress, disabled = false }: { label: string; onPress: () => void; disabled?: boolean }) {
  const { colors } = useTheme();
  const styles = useMemo(() => createStyles(colors), [colors]);

  return (
    <Pressable
      style={[styles.primaryButton, { backgroundColor: disabled ? hexToRgba(colors.primaryDark, 0.24) : colors.primaryDark }]}
      onPress={onPress}
      disabled={disabled}
    >
...
export function SubscriptionSetupSecondaryButton({ label, onPress }: { label: string; onPress: () => void }) {
  const { colors } = useTheme();
  const styles = useMemo(() => createStyles(colors), [colors]);
