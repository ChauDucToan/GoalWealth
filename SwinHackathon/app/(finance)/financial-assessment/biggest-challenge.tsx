...
import { biggestChallengeOptions } from '@/components/financial-assessment/data';
import { AssessmentPrimaryButton, AssessmentShell } from '@/components/financial-assessment/shared';

export default function FinancialAssessmentBiggestChallengeScreen() {
  ...
  const { state, setBiggestChallengeId } = useFinancialAssessment();

  return (
    <AssessmentShell
      step={19}
      totalSteps={21}
      eyebrow="Step 19 of 21"
      title="What are your biggest financial challenges?"
      body="Pick the challenge that feels most real today. It helps prioritize coaching and alerts after onboarding."
      footer={
        <AssessmentPrimaryButton
          label="Continue"
          disabled={!state.biggestChallengeId}
          onPress={() => router.push('/(finance)/financial-assessment/commitment-words')}
        />
      }
    >
      <View style={styles.stack}>
        {biggestChallengeOptions.map((item) => {
          const active = item.id === state.biggestChallengeId;
